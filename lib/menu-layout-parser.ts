...
function cleanWord(value: string) {
  return normalizeDigits(String(value || ""))
    ...
    .replace(/[／/]/g, "")
    .replace(/[：]/g, ":")
    .replace(/炒\s*8\s*球/g, "炒蝦球")
    .replace(/友\s*愛\s*熱\s*炒/g, "友愛熱炒")
    .replace(/十\s*還\s*膳/g, "十全藥膳")
    .replace(/\s+/g, " ")
    .trim();
}
...
function normalizeMenuLine(text: string) {
  ...
  const compact = cleanWord(text)
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (countCjk(compact) < 2 || compact.length > 16) return "";
...
function pushDefaultCategory(output: string[]) {
  if (!output.length) output.push("精選菜單");
}
...
export function parseOcrWordsToStructuredMenu(words: OcrWordBox[]) {
  ...
  const topRows = clusterRows(topWords, 24);
