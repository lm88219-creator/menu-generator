...
export default async function Page({ params, searchParams }: PageProps) {
  ...
  const id = await getMenuIdBySlug(decodedName);
  ...
  const themeStyles = {
    dark: {
      pageBackground: "radial-gradient(circle at top, #1d1d1d 0%, #050505 50%, #000 100%)",
      pageTextColor: "#fff",
    ...
    <main style={{ minHeight: "100vh", background: current.pageBackground, color: current.pageTextColor, padding: "24px 16px 60px", fontFamily: "Arial, sans-serif" }}>
      <div style={{ maxWidth: 860, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          <div style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", color: current.mutedColor, fontSize: 13, letterSpacing: 2, marginBottom: 14, padding: "8px 14px", borderRadius: 999, background: current.heroBadgeBg }}>DIGITAL MENU V2</div>
          ...
          <p style={{ marginTop: 10, color: current.mutedColor, fontSize: 15 }}>掃碼即看，手機友善的餐廳數位菜單</p>
        </div>
        <div style={cardStyle}>
